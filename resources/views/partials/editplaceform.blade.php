<div class="form-container" id="formContainer">
    <form id="registrationForm" action="{{ route('updateplace', $place->DEP_ID) }}" method="POST">
        @csrf
        @method('PUT')
        <label for="depName" class="form-check-label">Газар нэгжийн нэршил</label>
        <input type="text" class="form-control" id="depName" name="depName" value="{{ $place->DEP_NAME }}" required>
        ...
        <input type="number" class="form-check-input" id="sortOrder" name="sortOrder" value="{{ $place->SORT_ORDER }}"
            required>
        ...
        <select id="parentDepId" name="parentDepId" class="form-control" required>
            ...
            @foreach ($departments as $department)
                <option value="{{ $department->DEP_ID }}" {{ old('parentDepId', $place->PARENT_DEPID) == $department->DEP_ID ? 'selected' : '' }}>
                    {{ $department->DEP_NAME }}
                </option>
            @endforeach
        </select>
        ...
        <select id="directorEmpId" name="directorEmpId" class="form-control" required>
            ...
            @foreach ($employees as $employee)
                <option value="{{ $employee->EMP_ID }}" {{ old('directorEmpId', $place->DIRECTOR_EMPID) == $employee->EMP_ID ? 'selected' : '' }}>
                    {{ $employee->EMPNAME }} ({{ $employee->DEP_NAME }}-{{ $employee->POS_NAME }})
                </option>
            @endforeach
        </select>
